...
- (void)repeat {
    [self playItem:self.nowPlayingItem];
}

#warning changed by serejahh
- (void)stop {
    [self.player pause];
...
- (void)handleAVPlayerItemDidPlayToEndTimeNotification {
    for (id <GVMusicPlayerControllerDelegate> delegate in self.delegates) {
        if ([delegate respondsToSelector:@selector(musicPlayer:endOfSongReached:)]) {
            [delegate musicPlayer:self endOfSongReached:self.nowPlayingItem];
        }
    }
    return;
...
- (void)doUpdateNowPlayingCenter {
    ...
    if (!artworkImage) {
        artwork = [[MPMediaItemArtwork alloc] initWithImage:[UIImage imageNamed:@"placeholder"]];
    }
...
- (void)beginInterruption {
    ...
    for (id <GVMusicPlayerControllerDelegate> delegate in self.delegates) {
        if ([delegate respondsToSelector:@selector(musicPlayerHasInterrupted:)]) {
            [delegate musicPlayerHasInterrupted:self];
        }
    }
    [self pause];
}

//TODO: continue reading\playing
